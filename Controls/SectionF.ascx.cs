namespace ProjectPortfolio.Controls
{
	...
	public partial class SectionF : System.Web.UI.UserControl
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            btnAddProduct.Attributes.Add("onclick", "javascript:popupWindowProduct(" + nInitiativeID.ToString() + ", 0);");

            lnkIESArchApDate.HRef = "#";
            lnkIESArchApDate.Attributes.Add("onclick", "javascript: popupWindowDatePicker('" + txtIESArchApDate.ClientID + "'); return false;");

            hRefIESArchApproverID.HRef = "#"; 
            hRefIESArchApproverID.Attributes.Add("onclick", 
                                            "javascript:popupWindowSelectItem(1,'" +
                                                        txtIESArchApprover.ClientID + "','" +
                                                        hiddenIESArchApprover.ClientID + "');" + 
                                                        "return false;"
                                            );

            // make the text box read-only while still allowing it to submit its value
        ...
        private void LoadInitiative()
        {
            ...
            if (drInitiative != null)
            {
                ...
                ddlFullyCompliant.SelectedValue = drInitiative["IESFullyCompliantID"].ToString();

                txtIESFinancialJustification.Text = drInitiative["IESFinancialJustification"].ToString();
            }
        }
        ...
        public int UpdateInitiative()
        {
            ...
            int nIESArchApproverID=0;
            int nFullyCompliantID = 0;

            try
            {
                nIESArchApproverID = Convert.ToInt32(hiddenIESArchApprover.Value);
                nFullyCompliantID = Convert.ToInt32(ddlFullyCompliant.SelectedValue);
            }
            catch
            {
                nIESArchApproverID=-1;
                nFullyCompliantID = -1;
            }
            ...
            if (nInitiativeID > 0)
            {
                ...
                nReturnValue = SectionF_DB.UpdateInitiative_IESConformity(nInitiativeID,
                                              nIESArchApproverID,
                                              txtIESArchApprover.Text,
                                              txtIESArchApDate.Text != String.Empty ? (object)DateTime.Parse(txtIESArchApDate.Text) : DBNull.Value, 
                                              nFullyCompliantID,
                                              ddlFullyCompliant.SelectedItem.Text,
                                              txtIESFinancialJustification.Text);
        ...
        protected void repeaterProduct_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            else if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                ImageButton imgButt = (ImageButton)e.Item.FindControl("delProductID");
                if (imgButt != null)
                    imgButt.Attributes.Add("onClick", "javascript: if (confirm('Are you sure you wish to delete this product ?')) AllowOneTimeSubmit(); else return false; ");
        ...
        {
            int nProductID;
            int nTypeID = 3; //typeID=3 for product
            //items
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                if (e.CommandName == "Delete")
                {
                    try
                    {
                        nProductID = Convert.ToInt32(e.CommandArgument);
                    }
                    catch (Exception e1)
                    {
                        nProductID= -1;
                    ...
                    SectionF_DB.DeleteInitiativeProduct(nInitiativeID, nProductID, nTypeID);
                }
            }
